...
- Studio-owned `provider=session-review`가 unavailable이고 fallback이 native여도 같은 lease로 native reviewer를 dispatch하지 않는다. cached capability로 `review-lease-replan-required`를 재현하고 `provider=native`인 새 canonical lease를 요구한다.
