...
`review plan-next`가 고른 `allowed_commands`는 permit 후보 목록이다. 실제 물리 실행 전에
`references/execution-control.md` 절차로 exact command profile/permit을 고정하고 atomic claim을
얻는다. pairing/외부 worker의 문자열 verification 결과만으로 evidence pin을 만들지 않고,
permit-bound `command-receipt/v1`을 먼저 기록한 뒤 그 receipt를 source로 하는
`verification-evidence/v1`만 cycle event에 연결한다.
