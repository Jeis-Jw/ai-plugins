...
| allowed | forbidden |
|---|---|
| `mode status/start/end`, `init`, mission/budget/backlog/cast 준비 | `apply_patch`, `git apply`, 직접 파일 수정 |
| run spawn/wait/record, minutes/delta 보고 | track 변경을 main에 직접 반영 |
...
pairing output의 `readyForIntegration`이 true이고 owner가 승인해야만 integration으로 넘어간다.
