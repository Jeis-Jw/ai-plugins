...
const seeds = await parallel(
  PERSONAS.map((p, i) => () =>
    agent(personaTurnPrompt('', p, 'Open the room: give your independent take on the agenda. You cannot see the others yet.'),
      { schema: TURN_SCHEMA, label: `diverge:${p.name}`, phase: 'Diverge' })
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  ...
  for (const p of PERSONAS) {
    const turn = await agent(
      ...
      { schema: TURN_SCHEMA, label: `debate:r${round}:${p.name}`, phase: 'Debate' })
    if (!turn) continue
  ...
  const critique = await agent(criticPrompt(roundSubmitted),
    { schema: CRITIC_SCHEMA, label: `critic:r${round}`, phase: 'Debate' })
...
].join('\n'), { schema: SYNTH_SCHEMA, label: 'summarizer', phase: 'Converge' }) || { synthesis: '(summarizer failed)', minority: 'none', proposals: [] }
...
const verdict = await agent([
  ...
  '\n--- verified delta_log ---\n' + JSON.stringify(deltaLog, null, 2),
].join('\n'), { schema: VERDICT_SCHEMA, label: 'critic:final', phase: 'Converge' }) || { alive: deltaLog.length > 0, reason: 'critic unavailable; fell back to delta count' }
