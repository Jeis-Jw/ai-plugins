...
4. status 갱신 — `set-status`로 status block만 제자리 교체(나머지 섹션 보존)
   ...
   - 아래 JSON은 핵심 필드 예시다. 실제 호출은 기존 status 전체를 복사해 lease id,
     reviewer/ref/digest/timestamp/counter 필드를 보존한 뒤 위 필드만 바꾼다.
   ```bash
   ...
   ```
   fast mode는 `--slug` 대신 이전 reviewer가 돌려준 전체 `--status-json`을 사용하고,
   반환 status JSON을 다음 reviewer handoff로 그대로 넘긴다. address 불가 시
