...
1. snapshot 로드 및 gate
   ...
   # fast: snapshot이 없으므로 --slug 대신 --status-json '<final status JSON>'
   ```
...
2. worker 브리핑 확인
   ...
   reviewer lease의 `reviewed_ref`/`finding_digest`, fresh/reuse counters도 최종 판정과
   일치해야 한다. `validate-status`가 불완전한 lease를 거부하면 완료하지 않는다.
   fast mode는 같은 gate를 `validate-status --status-json`으로 실행한다.
3. receipt 출력
   snapshot discard 전(audit) 또는 최종 context status(fast)에서 schema v1 receipt를
   만든다. 하니스가 exact token 값을 주지 않으면 `--tokens`를 생략한다. 결과는
   `tokens:null`, `token_coverage:unavailable`이며 0으로 치환하지 않는다.
   ```bash
   python3 "$SR" emit-receipt --slug <snapshot> --run-id <review-run-id> \
     --started-at <workflow-started-at> --finished-at <workflow-finished-at>
   # fast: --slug 대신 --status-json '<final status JSON>'
   ```
4. 머지 전 확인
   ```bash
   git status --short
...
5. squash merge + snapshot discard 단일 커밋
   ```bash
...
6. fast self-review 완료
   `recording_mode=fast`는 snapshot/review branch가 없으므로 squash/discard를 하지 않는다.
   단, reviewer는 lease가 지정한 분리된 reviewer여야 한다. 현재 작업트리에 최종 변경만 남긴 뒤
...
- self turnkey도 same-agent self-check로 완료할 수 없다. lease로 분리된 reviewer의
