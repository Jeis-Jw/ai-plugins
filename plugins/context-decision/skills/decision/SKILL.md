...
4. direct capture는 `candidate prepare --candidate-id ... --commitment-evidence ...`로 exact candidate를 먼저 만든다. `candidate_id`는 result 연결 외 의미가 없고, `--search-term`에는 실제 대화 표현과 필요한 동의어만 넣어 이후 index recall을 돕는다. 이 candidate를 본 owner skill이 claim/decline/clarification한 뒤 `capture --candidate @file --attestation @file` 또는 `--decline-reason`/`--needs-clarification-reason`으로 넘긴다. CLI가 evidence나 attestation을 자체 발명하지 않는다.
