...
먼저 context-core 설치와 `context-common/v1` compatibility를 확인한다. host가 exact plugin inventory와 read-only doctor receipt를 준비한 뒤 `decision_cli.py init --host <codex|claude-code> --core-inventory @file --core-doctor @file --json`을 호출한다.
...
- ready 또는 absent이면 반환된 descriptor와 index seed를 active installed core의 public `context_cli.py bootstrap --descriptor @file --index-seed @file --json` surface에 그대로 전달한다. cache path를 추측하거나 core를 내장·복제하지 않는다.
