...
이 문서는 wiki 플러그인이 **함께 이동시키는 규약**이다(§15: mechanism / policy / agent entry / knowledge 4계층 중 **mechanism**). 운영 정책(언제·누가·무엇을 capture할지)은 별도 정본인 `wiki/ssot/agent-operating-model.md`(프로젝트마다 작성)에, 실제 축적 내용은 vault(`wiki/`)가 담는다. 본 문서는 "어떻게 운영하는가"의 정본 — 본 플러그인을 다른 프로젝트로 옮기면 본 문서도 함께 따라간다.

본 메커니즘의 설계 정본은 `wiki/ssot/plugin_definition_v1.md`. 본 문서는 그중 운영 규약을 압축한다.
...
## 2. Living vs Record (핵심 구분축)

| | **living** (ssot, runbook) | **record** (context/*) |
|---|---|---|
| 성격 | 현재 상태 | 시점 기록 |
| 정체성 | **주제** | **시점** |
| 갱신 | **제자리 수정** | **불변 + supersede** |
| 파일명 | `<slug>.md` | `<TYPE>-<YYYY-MM-DD-HHMMSS>-<slug>.md` |
| 폐기 | retire 아닌 **갱신**; 주제 소멸 시 삭제 | `retired/`로 물리 이동 + `retired_type` |
| 관계 작성 | **절대 안 함** (불변식) | 함 (§5) |
| stale 판정 | 시간 stale + changed-path-stale | supersede가 주된 판정. 시간/경로 stale은 타입별 (§7 표) |
...
- **예외: `relations.tasks`** = 외부 작업 시스템 참조 (`owner/repo#N` 등). 위키 파일 존재 검사 대상 아님, 형식 검증만.
...
|---|---|
| `init` | 폴더 구조·인덱스 뼈대 멱등 생성. 기존 노트 자동 등록. observation 폴더 포함. |
| `capture` | 타입·관계·허브 불변식, ID 결정성, 인덱스 파생 갱신, supersede 자동 처리. **타입별 필드 가드** (verified_at/affects_paths 적용 타입 강제). |
| `retire` | record만 물리 이동, top-level 생명주기 필드 부여, supersede 양방향 갱신, **successor가 active context/\* record인지 검증**, 인덱스 재파생 |
| `recall` | 토큰 효율(3-stage + 가드 + search_terms 매칭), retired 기본 제외, basename 정확 매칭, `--read a,b,c` 배치 read |
| `refresh` | 무결성 점검 13종 (`stale` · `supersede` · `broken-rel` · `task-ref` · `orphan` · `index` · `retired-in-index` · `active-ref-retired` · `tags` · `changed-path-stale` · `duplicate-basename` · `empty-lesson` · `schema`). **자동 수정은 화이트리스트만** (`--fix index,retired-in-index`). bare `--fix` 또는 그 외 인자는 거부 — 의미 판단이 필요한 수정은 사람·에이전트가 capture·Edit으로 명시 처리 |
...
| 계층 | 위치 | 담는 것 |
|------|------|---------|
| **mechanism** | 플러그인 (`plugin_definition_v1.md` + 본 문서) | 타입집합·ID포맷·frontmatter 스키마·관계 작성·생명주기·조회 단계 |
| **policy** | `wiki/ssot/agent-operating-model.md` (프로젝트별 정본, 별도) | agent 역할, 이벤트 흐름, leaf issue 규약, promotion triggers, agent별 capture 권한 |
| **agent entry** | 프로젝트 루트 `CLAUDE.md` / `AGENTS.md` | 정책 ssot로의 짧은 포인터 + 프로젝트별 튜닝 |
| **knowledge** | `wiki/*` | 실제 축적 내용 |

플러그인은 **mechanism**만 제공한다 — agent-neutral. agent별 운영 규약은 프로젝트가 policy 계층에서 결정한다. 본 플러그인은 CLAUDE.md를 소유하지 않고, `skills/wiki/references/claude-md-snippet.md`로 권장 스니펫만 제공한다.
