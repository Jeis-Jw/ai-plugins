...
| `snapshot` | 정식 graph로 승격하기 전 대화 맥락 체크포인트 관리. `save/list/search/load/discard`를 제공하며 `SNAP-<slug>.md` 파일을 `snapshot/` 루트에 저장한다. `recall`/관계 해소/`refresh --strict`/basename 중복 검사 대상이 아니다. 기본 저장은 slug 기준 제자리 갱신이고, 종료는 `snapshot discard <ref>` 삭제다(git이 이력을 보유). |
