...
├── task/                       ← 작업 브릿지 (제3 범주: 활성 / done/ / retired/)
...
**task 상태 = 경로**: 활성 `task/`, 완료 `task/done/`, 무효 `task/retired/`. `status` 필드 없음(경로가 정본). `complete`/`reopen`으로 활성↔done 이동. supersede 안 함. 외부 작업(이슈)과 1:1로 잇는 브릿지이며, 연동 시 외부 트래커가 상태 정본이고 `done/`는 그 투영(위키 CLI는 외부 트래커를 모름).
