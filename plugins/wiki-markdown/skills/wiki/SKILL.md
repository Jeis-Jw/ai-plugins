...
- `complete`/`reopen --json` → `moved_from`/`moved_to` (the lifecycle move), `updated_indexes` (rewritten), and `suggested_git_paths` (source+dest+indexes, deduped) — `git add` those to stage the move; no path computation needed. `--dry-run` previews the same with `dry_run: true` and moves nothing. (GitHub/branch/label/root-close detection is task-github's closeout, not this CLI.)
