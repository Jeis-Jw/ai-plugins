...
| `discard <basename>` | **Permanently delete** a graph node — the mistake-undo opposite of `retire` (which keeps the file). Exact basename only; refuses when other docs reference it (backlinks) unless `--force`; `--dry-run` previews (`backlinks`, `would_block`) without deleting; `--json` reports `backlinks`/`index_paths`. git retains history. |
