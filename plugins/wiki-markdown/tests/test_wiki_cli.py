...
class WikiCliTaskTests(unittest.TestCase):
    ...
    def test_task_refs_accept_worker_plain_and_github_prefixed(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._init(tmp)
            _, dec_id = self._seed_decision(tmp)
            r = run_cli("relate", dec_id, "--add-tasks",
                        "task-worker:release-graph,owner/repo#42,github:owner/repo#9",
                        "--json", cwd=tmp)
            self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
            ...
            self.assertIn("task-worker:release-graph", text)
            ...
            bad_worker = run_cli(
                "relate", dec_id, "--add-tasks", "task-worker:bad id", "--json", cwd=tmp
            )
            self.assertEqual(bad_worker.returncode, 4, bad_worker.stdout + bad_worker.stderr)
