...
새 워크스페이스를 git + GitHub로 부트스트랩한다. local provider config만 필요한 기존 프로젝트는 외부 mutation이 없는 `task-github:init`을 사용한다.
...
### Step 0.5. task-github local provider 초기화

Git/GitHub 상태와 무관하게 자기 config와 projection state를 먼저 멱등 초기화한다.

```bash
python3 "${TASK_GITHUB_ROOT:-$CLAUDE_PLUGIN_ROOT}/scripts/init_workspace.py" --json
```

충돌이면 `--force`를 자동 적용하지 않고 중단한다. 이 단계는 GitHub API를 호출하지 않는다.
...
grep -qxF ".task-github/local/" .gitignore || printf ".task-github/local/\n" >> .gitignore
grep -qxF ".task-worker/local/" .gitignore || printf ".task-worker/local/\n" >> .gitignore
grep -qxF ".task-worker.yml" .gitignore || printf ".task-worker.yml\n" >> .gitignore
python3 "${TASK_WORKER_ROOT:-$CLAUDE_PLUGIN_ROOT/../task-worker}/scripts/task_config.py" scaffold --json
git add .
...
### Step 5. 안내
- 생성된 레포 URL 출력
- `.gitignore`에 `.worktrees/`, `.task-worker/local/`, `.task-github/local/`이 포함되어 있는지 확인
- `.task-worker.yml`에 mode/dispatch/delivery/orchestrate/evidence/recovery 실행 정책이 있는지 확인
- `.task-github.yml`에는 base_branch/projection/closeout GitHub 정책만 있는지 확인
- (운영 정책 미설정 시) `wiki-markdown`의 `agent-policy` 스킬로 `CLAUDE.md`/`AGENTS.md` operating policy 블록 생성 권장
- 다음 단계 안내: `task-github:define` 또는 `task-github:start`

## 불변식
- 멱등에 가깝게: 이미 구성된 환경은 덮어쓰지 않고 중단.
- task-github config는 자기 `init_workspace.py`로 준비하고, 다른 기존 config를 자동 덮어쓰지 않는다.
- task-worker는 기존 config scaffold bridge를 유지한다. task-github setup이 task-worker의 public init API를 직접 호출하지 않는다.
- 레포는 기본 **private**.
- 위키 init은 제안만 — setup이 강제로 vault를 만들지 않는다.
- task-worker config scaffold가 실패하면 GitHub adapter만 부분 구성하지 않고 중단한다.
