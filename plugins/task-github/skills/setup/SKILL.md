...
### Step 5. 안내
- 생성된 레포 URL 출력
- `.gitignore`에 `.worktrees/`와 immutable DefinitionArtifact/local run/projection checkpoint용 `.task-github/local/`이 포함되어 있는지 확인
- `.task-github.yml`에 `mode`/`base_branch`/tool/`orchestrate.*` 기본값이 있는지 확인
