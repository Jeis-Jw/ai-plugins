...
외부 `workflow-review-lease/v1`이 있는 edge에서는 `review-mode=skip`으로 FF closeout을 만들지 않는다. external owner여도 PR 생성·base/head transport·CI/preflight·`review_waiting`은 유지하고 reviewer dispatch만 억제한다. 동일 lease의 approved verdict와 필수 evidence가 ledger에 기록되기 전에는 `ready_for_pr_closeout`이나 merge를 수행하지 않는다.
