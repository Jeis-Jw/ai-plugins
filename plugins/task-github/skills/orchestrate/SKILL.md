...
python3 "${TASK_GITHUB_ROOT:-$CLAUDE_PLUGIN_ROOT}/scripts/task_worker_bridge.py" preflight
```
...
- `TASK_WORKER_ROOT` — preflight가 반환한 compatible task-worker 절대경로. 모든 local lifecycle·ready/integration 판단이 같은 versioned core를 사용하게 한다.
...
- `workflow-review-lease/v1`이 있으면 `ORCHESTRATE_REVIEW_MODE=skip`보다 우선한다. 외부 lease를 review 생략 근거로 사용하지 않는다.

```bash
# 오케스트레이터가 자기 루트를 절대경로로 1회 해소
TG_ABS="${TASK_GITHUB_ROOT:-$CLAUDE_PLUGIN_ROOT}"
TW_ABS=$(python3 "$TG_ABS/scripts/task_worker_bridge.py" preflight | python3 -c 'import json,sys; print(json.load(sys.stdin)["root"])')
ORCH_ENV="ORCHESTRATED=true TASK_GITHUB_ROOT=$TG_ABS TASK_WORKER_ROOT=$TW_ABS \
...
external-owner review도 위 PR/CI/preflight/transport를 그대로 만든다. 달라지는 것은 task-github reviewer dispatch 하나뿐이다. ledger의 external handoff는 `episode_id`, `edge_id`, `evidence_refs`, issue/PR/base/head를 보존한다. caller가 동일 lease의 `approved` verdict와 lease가 요구한 evidence refs를 반환하기 전에는 `ready_for_pr_closeout`, merge, closeout resume을 모두 거부한다. `changes-requested`는 caller로 되돌리고 task-github가 임의 worker/reviewer를 소집하지 않는다.
