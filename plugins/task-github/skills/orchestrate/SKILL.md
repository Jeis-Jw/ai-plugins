...
- `skip`: 모든 PR 자동 머지.
- `gear`: `gear:micro|normal`은 자동 머지 후보, `gear:major`만 review 필요.
...
`gear:normal`은 worker verification + CI success + mergeState CLEAN이면 review 없이 merge한다. `gear:major`도 review 요청 전에 완료조건/런타임 evidence를 먼저 sanity check하고, 불가능하면 scope split/follow-up/blocker를 만든 뒤 review한다.
