# orchestrate — 이슈트리 절차적 자동수행 (기획 드래프트 r5)

> 상태: 기획 드래프트 round 5 (브랜치트리 + 에이전트 오케스트레이션 co-design 확장 — §11).
> 범위: **고정 이슈트리 실행만.** 이슈 분해(define/brainstorm)는 범위 밖.
> **용어(r5):** "root"는 **루트 이슈/브랜치**만 가리킨다. 루프를 구동하는 메인스레드 역할은
> **오케스트레이터**라 부른다(§1~§9의 "root (main thread)" 표기는 오케스트레이터로 읽는다).
