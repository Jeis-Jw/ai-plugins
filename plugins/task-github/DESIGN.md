...
> - **`wiki/ssot/agent-operating-model.md`**: 위키↔task **결합 규약(policy)** — 이 플러그인 밖, 위키 vault 안에 산다([§2](#2-4계층에서-task-github의-위치) 4계층 분리)
>
> 충돌 시 신뢰 순서: **실행 명세(SKILL/rules/agents) > DESIGN > README**. 결합 규약(policy)은 mechanism(이 문서)과 **다른 계층**이므로 경쟁하지 않고 보완한다.

> **상태**: 이 문서가 기술하는 v3 설계는 **구현 완료**됐다(위키 CLI `task` 타입, task-github 스킬·룰·에이전트, 위키 policy 반영, 루트 `CLAUDE.md`). 구현 순서·현황은 [§19](#19-구현-순서-완료됨) 참조. 위키 task 설계는 vault에 dogfood로도 기록돼 있다([§6.1](#61-위키-task-노드--설계-확정)).
...
- **계층 위치**: 이 플러그인은 **agent-neutral 메커니즘**이다. "언제·누가·무엇을 캡처/연결하는가"의 운영 규약은 위키의 **policy 계층**(`agent-operating-model.md`)에 둔다.
...
이 플러그인은 진공에서 설계되지 않았다. `wiki-markdown`의 정책 정본 `wiki/ssot/agent-operating-model.md`가 **이 플러그인을 위한 자리를 명시적으로 예약**해두었다:
...
| **policy** | `wiki/ssot/agent-operating-model.md` (위키 vault) | 위키↔task 결합 규약: 캡처 권한, task 노드↔이슈 연결 포맷, promotion 트리거, PR 리뷰 흐름 ([§13](#13-위키-policy-슬롯에-채울-내용-operating-model)) |
| **agent entry** | 루트 `CLAUDE.md` / `AGENTS.md` | 프로파일 명시 + policy로의 짧은 포인터 |
| **knowledge** | `wiki/*` | 작업이 낳은 실제 노드(`task` + decision/trial_error/observation 등) |

> **왜 결합 규약을 플러그인이 아니라 위키 policy에 두는가**: 위키는 이미 "plugin은 구조 검증만, 의미 판정은 operating model"이라 결정했고(`DEC-…-promotion-threshold-in-plugin-spec`), "plugin CLI/스키마에 특정 도구 이름 없음"을 불변식으로 박았다(`DEC-…-plugin-agent-neutral-cli-schema`). task-github의 결합 규약(예: "define이 루트 이슈를 만들 때 task 노드를 함께 만든다")은 **변동 자산**(운영하며 바뀜)이고 **위키 타입에 의존**한다. 이를 task-github의 rules에 하드코딩하면 안정 자산과 변동 자산이 한 곳에서 함께 흔들린다 — 위키가 `TRI-…-claude-md-as-policy-conflates-mechanism-and-policy`에서 이미 겪은 함정이다.

**경계의 실제 의미**:
- task-github의 SKILL/rules는 "**위키가 있으면** task 노드를 만들어 이슈와 잇고, 관련 지식을 recall하고, 발견한 결정을 capture **하도록 위임**한다"까지만 쓴다 — *어떤 타입으로, 누가, 어떤 임계로* 연결·승격할지의 구체 규약은 policy를 참조한다.
...
**3계층 구조**(rules=헌법 / skills=함수 / agents=외부감사)는 v2와 동일. 신규는 `rules/wiki-bridge.md` 하나 — 위키 **감지·task 노드 연결·호출의 메커니즘**만 담고, *결합 정책*은 위키 policy를 가리킨다.
...
```json
{ "name": "task-github", "source": "./plugins/task-github", "version": "3.0.0",
...
| relations | `intents` / `decisions` / `ssot` / `tasks`(외부 이슈 ref) |
| ID | `TASK-YYYY-MM-DD-HHMMSS-<slug>` |
| 경로 | `wiki/task/` (완료 시 `wiki/task/done/`) |
| 상태 | **이진**: 활성 vs 완료(경로 이동) |
| CLI | `capture task` / `complete` / `reopen` (위키 CLI v0.3.0에 구현됨 — [§6.8](#68-위키-cli-task-지원-구현-완료-v030)) |

### 6.2 업무 ↔ 이슈 1:1 다리 (핵심 자료구조)

**한 업무 = task 노드 1개 + 루트 이슈 1개.** 둘이 서로를 가리켜 양방향 다리를 이룬다.

- **task 노드 → 이슈**: `relations.tasks: [owner/repo#N]` (루트 이슈 번호). task는 record 성질이라 이 역링크를 **가질 수 있다** — 이것이 task 노드를 신설한 핵심 이유다(아래 박스).
- **task 노드 → 결정/취지**: `relations.decisions`, `relations.intents` (이 업무가 어떤 결정·취지에서 나왔나 = 보조 컨텍스트의 정본).
- **이슈 → task 노드**: 루트 이슈 본문 `## Wiki Context`에 task 노드 basename을 **메인**으로, 관련 결정을 **보조**로 기록.

> **왜 task 노드가 비대칭 문제를 푸는가**: `intent`/`ssot`는 hub(또는 living)라 `relations.tasks` 역링크를 못 가진다. 그래서 "에픽 ↔ intent"를 직접 이으려 하면 단방향이 된다. `task`는 record 성질이라 이슈 링크를 들 수 있어 **양방향 다리**가 된다. 역방향 조회("이 결정이 낳은 업무는?")는 task의 outbound 관계를 **파생 백링크**(`recall --backlinks-of DEC-x`)로 잡으므로, decision/intent 스키마는 손대지 않는다(순수 가산).
...
| 본질 | **왜 이 업무가 생겼나** (결정·취지 묶음) | **무엇을·어떻게** (실행) |
...
|------|-----------|---------------------|
| **독립** (위키만, 작업 플러그인 없음) | 위키 | (해당 없음 — 이슈가 없음) |
| **연결** (task-github 사용) | **GitHub 루트 이슈** | 이슈 close 시 task 노드 `complete`(→`done/`). 밖에서 닫힌 경우 reconcile |
...
> 구체적 임계·권한(누가 어떤 타입을 언제)은 mechanism이 아니라 **policy**의 몫이다([§13](#13-위키-policy-슬롯에-채울-내용-operating-model)). 이 표는 mechanism이 보장하는 **기본 강도**만 규정한다.
...
### 6.8 위키 CLI task 지원 (구현 완료, v0.3.0)

위키 CLI(`wiki_cli.py`, v0.3.0)는 `task` 타입과 `complete`/`reopen`을 **구현 완료**했다:
...
## 13. 위키 policy 슬롯에 채울 내용 (operating-model)

> **이 절은 mechanism이 아니라 policy다.** 합의되면 이 내용을 `wiki/ssot/agent-operating-model.md`의 "구성요소"(현재 placeholder)에 반영한다. 위키의 4계층 분리상 이 규약은 위키 vault에 살아야 하며, 이 문서에는 **참조용 초안**으로만 둔다.
...
- **루트 이슈** 본문 `## Wiki Context`: task 노드를 **메인**, 결정/취지를 **보조**로 (포맷 [§6.2]).
...
- 연결 시 **GitHub 루트 이슈가 정본**, 위키 task 상태는 투영([§6.5]).
...
**새 위키 연동**: mechanism(이 플러그인)에는 **감지·호출 방법**만, **정책**(언제·어떤 타입)은 policy에. 위키 타입/CLI가 바뀌면 [§6](#6-위키-통합-핵심-장)·[§13](#13-위키-policy-슬롯에-채울-내용-operating-model)·`rules/wiki-bridge.md` 동기. 타입별 관계 제약(불변식 23)은 위키 `wiki-data-model`을 정본으로 재확인.
...
### 18.2 대상 프로젝트 1회 설정
```bash
echo "프로파일: solo" >> CLAUDE.md      # 또는 team
# 기어 라벨(필수)
...
# CLAUDE.md에 policy 포인터 + wiki/ssot/agent-operating-model.md에 §13 반영
```
...
3. ✅ **policy 반영** — [§13]을 `wiki/ssot/agent-operating-model.md`에 반영 + 루트 `CLAUDE.md`에 프로파일·policy 포인터.
...
1. **계층 분리 도입**: 위키↔task 결합 규약을 플러그인이 아니라 위키 **policy 계층**(`agent-operating-model.md`)에 둔다(위키 4계층 분리 존중, 불변식 19).
...
| 프로파일 | 환경 분류(solo/team). `CLAUDE.md`에 명시(기본 solo). |
...
| `task` 노드 | 위키 제3 범주. 업무 요약·근거를 담고 루트 이슈를 가리키는 다리. 이진 상태(활성/done). |
...
| mechanism/policy | 안정 자산(이 플러그인) / 변동 자산(위키 operating-model의 결합 규약). |

---

*이 문서는 `wiki-markdown` 플러그인 명세(`wiki/ssot/plugin-definition/`, `skills/wiki/`, `wiki_cli.py`)와 vault에 dogfood된 task 설계(`DEC-…-task-third-category` 등), v2 `task-github` 실행 명세를 분석해 작성되었다. 위키 타입/CLI가 바뀌면 [§6](#6-위키-통합-핵심-장)·[§13](#13-위키-policy-슬롯에-채울-내용-operating-model)을 갱신하라 — 위키 `wiki-data-model`이 타입·관계의 정본이다.*
