...
1. **선택적 GitHub 기록** — `record:github`/legacy mode에서는 Issue/Label/Assignee/PR이 실행 기록이다. `record:none`은 `.task-github/local/`의 DefinitionArtifact와 run state를 사용하고 GitHub Issue를 만들지 않는다. GitHub projection도 재시도용 local checkpoint를 사용하며 별도 DB는 두지 않는다.
