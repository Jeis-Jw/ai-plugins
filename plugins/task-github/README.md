...
머지 경로는 gear가 결정한다(DEC-2026-07-02-224910). **micro/normal 리프**는 PR 없이 `done`이 로컬 FF(`git fetch . task/issue-{leaf}:task/issue-{parent}`, checkout 없는 refspec)로 부모에 합류하고, close 증거로 verify 리포트 + 커밋 SHA range를 남긴다. **major 리프**와 **누적 gear가 major인 컨테이너**만 `closeout.py --pr {PR}` → `gh pr merge`(remote) 경로를 탄다. 컨테이너 gear는 자식 누적 승격(`container_gear_promotion`: micro×3→normal, normal×2→major)으로 결정되므로, 작은 작업이 쌓여도 trunk 도달 전 어딘가에서 반드시 PR+리뷰 게이트를 통과한다. closeout은 로컬 `git checkout`/`git merge`를 하지 않고 base 갱신도 `git fetch origin {base}:{base}`로 처리해, 오케스트레이션 중 메인 워크트리 HEAD가 trunk를 벗어나지 않는다(DEC-2026-07-02-212109 불변식 유지). 충돌은 항상 리프 worktree에서 해소한다.
