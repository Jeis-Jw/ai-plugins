...
Studio가 외부 executor인 경우 task-github는 task-worker binding의 `workflow-review-lease/v1`을 하나의 구조화 계약으로 소비한다. `owner=studio`에서도 PR 생성, CI/preflight, `in-review`/`review_waiting`, base/head transport와 closeout lane은 유지하고 reviewer tool/harness dispatch만 억제한다. ledger가 lease/episode/edge/evidence/issue/PR/base/head를 보존하며, 동일 lease의 approved verdict와 필수 evidence가 돌아오기 전에는 `ready_for_pr_closeout`, merge, closeout resume이 모두 fail-closed한다. lease 없음이나 `owner=task-worker`는 기존 review-tool/human gate/feedback loop를 유지한다.
