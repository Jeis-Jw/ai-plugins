...
### task-worker DefinitionArtifact local lifecycle
...
- canonical artifact는 provider `record`를 저장하지 않는다. task-github facade의 local-only 선택은 GitHub Issue write를 금지하며, delivery는 `local-ff|external` 중 하나다.

## 외부 review owner

task-github는 task-worker binding의 exact `workflow-review-lease/v1`을 versioned preflight로 소비한다. issue edge마다 `review-expectation`의 lease를 tick/review/closeout 전에 ledger `expected_review_lease_pinned`으로 먼저 고정한다. 동일 pin 재적용은 멱등이고 mismatch/rebind는 fail-closed다. expected lease가 있는데 permit이 없거나 다르면 local reviewer도 호출하지 않는다. `owner=task-worker`가 아닌 opaque external owner면 PR·CI·preflight·base/head transport·`in-review`/`review_waiting`·closeout lane은 유지하고 reviewer tool/harness dispatch만 금지한다. handoff와 verdict는 ledger에 lease/episode/edge/evidence/issue/PR/base/head를 남긴다. public closeout/merge/resume은 handoff 기록 순서와 무관하게 pinned lease의 동일 approved verdict와 required evidence를 요구하며 `review-mode=skip`은 외부 lease를 우회하지 못한다. lease 없음이나 valid `owner=task-worker`는 기존 review 흐름이다.
- GitHub 기록 선택은 root, 모든 descendant, 모든 dependency edge가 projection checkpoint에 materialize된 뒤에만 Issue 실행을 허용한다. partial projection은 같은 artifact digest로 resume한다.
