...
task-github는 task-worker binding의 exact `workflow-review-lease/v1`을 versioned preflight로 소비한다. `owner=studio`면 PR·CI·preflight·base/head transport·`in-review`/`review_waiting`·closeout lane은 유지하고 reviewer tool/harness dispatch만 금지한다. handoff와 verdict는 ledger에 lease/episode/edge/evidence/issue/PR/base/head를 남긴다. 동일 lease의 approved verdict 전에는 closeout을 fail-closed하며 `review-mode=skip`은 외부 lease를 우회하지 못한다.
