...
- 분해 단위 간 touched/affects path가 겹치고 dependency가 선언되지 않음 — 단, **재합침이 먼저다**: 겹치는 형제가 same-theme(같은 기반을 여러 표면에 적용)이면 dependency로 직렬화하기 전에 **1리프 + phase 체크리스트로 합치는 것**을 먼저 검토한다(define §절단 원리 겹침 처리 우선순위). 테마가 다른데 경로만 겹칠 때만 `blocked_by`로 승급한다.
