...
physical command 전에 execution fingerprint를 조회한다. 동일 definition/node/HEAD/command/environment/tool version의 성공 evidence가 있으면 실행하지 않고 기존 evidence id를 verify event에 연결한다.

```bash
python3 "${TASK_WORKER_ROOT:-$CLAUDE_PLUGIN_ROOT}/scripts/definition_artifact.py" evidence-plan \
  --request {EVIDENCE_REQUEST_JSON} --state-root .task-worker/local
```

evidence에는 최소한 `head`, `paths`, `command_digest`, `environment_digest`, `tool_version`, `result`를 기록한다.
