---
name: start
description: DefinitionArtifact의 ready leaf를 stable branch/worktree identity에 pin한 local run으로 시작한다. container와 열린 blocker는 차단한다. "task-worker:start", "이 작업 시작해", "artifact node 시작" 요청에 사용한다.
---
...
python3 "${TASK_WORKER_ROOT:-$CLAUDE_PLUGIN_ROOT}/scripts/definition_artifact.py" local-start \
  --artifact {ARTIFACT} --node {KEY_OR_NODE_ID} --state-dir .task-worker/runs
```
