...
- B1: leaf gear는 define/분해 시점 선결 → ready_leaves가 gear 싣고 루프가 spawn 전 gear:major STOP. 실행주체 = create_issue_tree.py child spec에 gear 필드 + execute에서 gh issue edit --add-label gear:{v}. missing/multiple/unknown gear는 default 없이 invalid_gear[] STOP.
- B2: stuck[] = in-progress 리프 ∩ NOT(active spawned worker), 결정론적. GitHub만으로는 판정 불가하므로 root 루프가 spawned_set/failed_set을 ready_leaves.py에 넘긴다. 자동재시도 금지, 사람게이트 STOP. reason: prior_run|spawned_failed.
...
- C3: 컨테이너 close는 root가 done_containers로 직접. closeout.py 위임 아님. closeout은 root-close 감지만.
...
- D3: 진행 단조성(closed leaf+done_containers 증가) 가드 + max-iter backstop. 분기순서 stuck>done_containers>review_waiting>invalid_gear>spawn.

## 배경

정본 설계 = plugins/task-github/skills/orchestrate/PLAN.md (main 9c9d188 기준에서 fast self-review 보정 반영). 2라운드 co-design 리뷰 approved 뒤, 구현 전 fast self-review에서 blocker 1개를 찾아 PLAN r3로 보정함.
...
1. scripts/ready_leaves.py — root# + spawned_set/failed_set → {ready[](gear포함), blocked[], review_waiting[], invalid_gear[], stuck[], done_containers[], root_done}. open/closeout prior art 재사용. self-check fixture(§4 예시트리 tick0 ready=={1-1,2-1-1,2-2}).
2. skills/orchestrate/SKILL.md — root 루프(PLAN §5.2). worker=기존 start/run/done 재사용. 사람게이트.
3. create_issue_tree.py — child spec gear 필드 + gear 라벨 부여(B1 실행주체). 같은 커밋에서 define/SKILL.md spec 예시와 기존 "기어 라벨 안 붙임" 불변식도 orchestrate tree 예외로 갱신.
