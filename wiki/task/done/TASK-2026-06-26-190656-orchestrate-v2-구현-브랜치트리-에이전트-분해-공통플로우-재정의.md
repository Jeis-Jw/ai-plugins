...
> **이번 구현 acceptance = §11.4 v1.** 자동 reviewer/conflict-agent는 같은 설계의 타겟과 후속 슬롯으로 문서화하지만, 이 task의 첫 ship gate는 v1 완료다.

- **스파인 정돈(v1)**: start(점유만)·run(worktree)·done(PR+부모base+in-review)·merge(PR merge+close; 충돌해소는 v2)
- **트리 선행 정돈(v1)**: define(dep 필수 materialize)·open(부모/서브이슈 트리관계 브리핑)
- **신규(v1)**: `skills/orchestrate/`(SKILL.md 루프 + `ready_leaves.py`) · 워크스페이스 루트 `.task-github.yml`(정본 config)
- **v2 후속 슬롯**: reviewer-agent(configured review-tool) · conflict-agent(test-gated)
- **재사용(v1)**: work-agent=Agent로 start→run→done / 위임=`*-tool` config
...
- 출력 JSON:
  - `ok` — false면 진행 불가. **모든 STOP은 단일 `stop_reason` 채널로 흐른다(C6 — r4 신규)**:
    enum `human_gate_review | merge_conflict | state_mismatch | stuck | dep_cycle |
    api_failure | no_progress | empty_tree`. `stop_reason`는 helper·오케스트레이터 공유 STOP **채널**이다 — 모두 ready_leaves가 내는 게 아니다(F1 producer 구분):
    - **helper-raised**(ready_leaves walk로 판정): `human_gate_review`·`stuck`·`dep_cycle`·`api_failure`·`no_progress`·`empty_tree`.
    - **orchestrator-raised**(머지/close 단계서 판정 — ready_leaves는 PR 상태 조회 안 함): `merge_conflict`(gh pr merge 충돌)·`state_mismatch`(아래).
    (`merge_conflict` = F7: stuck/no_progress에 섞지 않음. conflict-agent 미가동 v1·의미적 모호 충돌이 이 reason.)
    `state_mismatch` = issue/PR/branch 상태가 완료 신호와 맞지 않는 복구 필요 상태(§11.3.9).
    분기별로 8가지 shape를 SKILL 루프가 각각
...
loop:
  ...
  r.container_done  → PR/close 증거 guard 후 부모 있으면 부모 브랜치로 merge+close, 부모 없으면 base_branch로 merge+close+위키 처리
  r.done_parents    → parent별 열린 blocked_by/API 실패 + PR/close 증거 guard 후 오케스트레이터가 merge+close,
...
8. **tool/command 매핑(v2)**: 루트의 `verify-tool`/`review-tool`은 어떤 어댑터·스킬을 쓸지
   정하고, orchestrate의 `verify-command`/`review-command`는 자동 모드에서 붙일 인자만
   정한다. 사람 드리븐은 `*-tool`을 기준으로 사용자에게 묻고, orchestrate는
   `/{verify-tool} {orchestrate.verify-command}` 또는 `/{review-tool}
   {orchestrate.review-command}`를 호출한다. review changes-requested →
   work-agent 재spawn(address-feedback handoff 주입) → 라운드 캡 초과 STOP.
...
- **설정 파일 `.task-github.yml` (워크스페이스 루트, 신규)** — 정본 config. 값은 영어, 기본 `solo`/`gear`.
  ...
  review-tool:             # 플러그인/스킬 이름 — 비면 하네스/사람이 review 수행
  orchestrate:
    verify-command:        # optional args appended to verify-tool in orchestrate mode
    review-mode: gear      # gear | all | skip — orchestrate 전용(기본 gear)
    review-command:        # optional args appended to review-tool in orchestrate mode
  ```
  - `base_branch`는 필수다. GitHub default branch를 추론하지 않는다(예: `main`, `master`, `develop`).
  - `mode`는 `solo|team` 운영모드만 표현한다. session-review의 `self` 같은 tool 인자는 `*-command`에만 둔다.
...
서브에이전트 (LLM, 판단):
  ...
  reviewer-agent : PR 리뷰 → verdict/findings를 오케스트레이터에 return.
...
**11.3.5 PR 리뷰 = review-tool + review-command relay (v2)**
- 루트 `review-tool`은 review 어댑터/스킬을 정한다(예: `session-review:request-review`).
- `orchestrate.review-command`는 orchestrate 자동 모드에서만 `review-tool` 뒤에 붙이는 인자다
  (예: `self turnkey`). 비어 있으면 tool 기본값으로 실행하거나, 도구가 자동실행을 지원하지 않으면 사람/하네스 STOP이다.
- 사용자가 직접 리뷰를 드라이브하는 단일 PR/수동 모드에서는 `review-tool`을 기준으로 사용자에게 물어보고 진행한다. orchestrate가 특정 review profile을 강제하지 않는다.
- orchestrate 자동 모드에서 `review-tool: session-review:request-review` + `orchestrate.review-command: "self turnkey"`로 설정된 경우:
  - work-agent=session-review의 worker(리프 소유), reviewer-agent=reviewer, 오케스트레이터=worker-side relay(request-review 호출·feedback 중계 — 리프 라벨은 안 만짐, D2 유지).
  - fresh reviewer subagent를 띄우므로 same-agent self-check가 아니다.
  - turnkey fast profile이라 snapshot/review branch/round commit은 만들지 않는다.
- 호출 shape: `/{review-tool} {orchestrate.review-command}`.
- review-tool은 verdict와 findings를 오케스트레이터에 return한다. GitHub PR comment/영속 피드백을 남길지는 review-tool 계약이다.
- 오케스트레이터는 review feedback을 직접 수정하지 않고, changes-requested면 **work-agent 재spawn**으로 넘긴다
  (handoff = PR#·브랜치·피드백). 라운드 캡 초과 → 사람 STOP.
- verdict approved → 오케스트레이터가 머지.

**11.3.5b verify = verify-tool + verify-command relay (v2)**
- 루트 `verify-tool`은 verify 어댑터/스킬을 정한다.
- `orchestrate.verify-command`는 orchestrate 자동 모드에서만 `verify-tool` 뒤에 붙이는 인자다.
- 호출 shape: `/{verify-tool} {orchestrate.verify-command}`.
- 단일/사람 드리븐 플로우에서는 `verify-tool`을 기준으로 사용자에게 묻고 진행한다.
- verify는 같은 작업 세션의 자기검증 단계이므로, review처럼 feedback rework loop를 소유하지 않는다.
- `session-review:request-review`의 `self`는 fresh reviewer subagent를 뜻한다. 순수 same-agent self-check가 필요하면 `verify-tool`을 하네스 self-check/default verify로 둔다.
...
- **PR 재발견 규칙**: 새 PR 생성 전 항상 `head=task/issue-{N}`, `base=base_branch(issue N)`의 기존 PR(open/merged)을 조회한다. open PR이 있으면 재사용, merged PR이면 issue close 여부만 보정, 없을 때만 생성한다. 중복 PR 생성 금지.
  - **stale-base 가드(F4)**: v1은 `task/issue-{N}` 명명이 불변이라 base churn 거의 없지만, base가 바뀌면 `--base` 필터가 stale-base PR을 놓쳐 중복 생성할 수 있다 → `head`-only 재조회로 stale-base open PR 감지 시 `state_mismatch` STOP.
- **merge/close 복구 규칙**:
  - PR merged + issue open → `gh issue close`만 재시도.
  - issue closed + PR unmerged/open → `state_mismatch` STOP(수동 close 또는 실패 복구 필요).
  - parent/container PR created 후 세션 종료 → 재실행 시 PR 재발견 규칙으로 이어간다.
- **완료 신호 guard**: `subIssuesSummary.completed`만으로 부모/컨테이너를 코드 통합 완료로 보지 않는다. 각 child가 no-change close이거나, 해당 child PR이 expected base에 merged된 증거가 있어야 merge+close를 진행한다. 증거가 없으면 `state_mismatch` STOP.
  - **증거 데이터 경로(F2)**: child별 `gh pr list --head task/issue-{child} --base task/issue-{parent} --state merged`(PR 재발견 규칙과 동일 probe) 또는 no-change close 라벨. **이건 net-new 오케스트레이터 로직** — `closeout.py`의 `_parent`/`_detect_root_task`는 child PR merge target을 안 준다(per-child PR base 조회 미제공).
...
- **v2**: STOP 슬롯에 투입 — reviewer-agent(= `review-tool + orchestrate.review-command`) + conflict-agent
  (test-gated). 리뷰정책 옵션의 `all`/자동 fix루프가 여기서 의미 가짐.
- 단계는 빌드 순서 권고일 뿐, flow 정본은 §11.3.

### 11.5 전체 flow 의사코드 (v1 스파인 + v2 슬롯 명시 — F3)

> v1 구현은 `[v2]` 태그 줄을 STOP으로 대체한 것이다. §5.2는 분기/STOP 골격, 이 블록은 브랜치트리·머지·guard까지 합친 정본.
```
  ...
  오케스트레이터.loop:
    ...
    ready 리프:
      ...
      work-agent spawn (start→run→done까지, 머지 안 함):
        ...
        → run 워크트리(부모브랜치 base)
          → [gear: plan → ] 수행 [ → verify-tool + orchestrate.verify-command ]   # micro=수행만 / normal·major=plan·verify 포함
      ...
      리뷰 필요?  (all→예 / skip→아니오 / gear→micro:아니오·normal|major:예)
        ...
        예     → [v1] STOP(human_gate_review) — 다음 tick review_waiting로 사람 리뷰·머지, 재실행 재개
              → [v2] reviewer-agent(review-tool + orchestrate.review-command) → verdict/findings
                       approved → gh pr merge → gh issue close
                       changes-requested → 오케스트레이터가 work-agent 재spawn(handoff) → 반복 / 캡 초과 STOP
      gh pr merge 실패(충돌) → [v1] STOP(merge_conflict)  /  [v2] conflict-agent(test-gated) → 재시도
    container_done:
      PR 재발견/merge-close guard 통과 후:
...
| **review** | 유지(소폭) | **작업과 별개 세션의 독립검증**. 수동/사용자 드리븐이면 `review-tool` 기준으로 사용자에게 묻고 진행. orchestrate 자동 모드에선 `review-tool + orchestrate.review-command`로 reviewer-agent relay, 도구가 없으면 사람/하네스 STOP | verify와 **방법 차이 아니라 단계+태도 차이**: review=별개 세션 독립검증 |
...
| **verify** | 유지(태도 명시) | **작업과 같은 세션의 자기검증**(self). 수동/사용자 드리븐이면 `verify-tool` 기준으로 사용자에게 묻고, orchestrate 자동 모드에선 `verify-tool + orchestrate.verify-command`. 비면 하네스 self-check | review와 단계+태도 차이: verify=같은 세션 자기검증 |
| **setup** | 유지 + **신규 config 핸들링** | git init/repo 생성 + **`.task-github.yml` scaffold**(YAML write — 기존 config 경로 없음, net-new): `mode`/`base_branch`/tool/`orchestrate.verify-command`/`orchestrate.review-mode`/`orchestrate.review-command` 기본값 |
| **doctor** | 유지 + **신규 config 핸들링** | linkage 진단 + **`.task-github.yml` validate**(YAML parse + 키/값 유효성 + tool-name 해소 — net-new, 기존 doctor.py에 config 핸들링 없음) |
...
| 산출물 | 종류 | 이유 |
|--------|------|------|
| **orchestrate** | 신규 스킬(빌드) | 현재 PLAN.md만 존재. 루프 + `ready_leaves.py` + 에이전트 spawn + gear 기반 정책 반영 + 머지/close + 게이트 STOP + 루트완료 위키 — 본 설계의 핵심 산출물 |
...
| **reviewer-agent** | `review-tool + orchestrate.review-command`(v2) | review 스테이지 = 독립검증. session-review 사용 시 `review-tool: session-review:request-review` + `review-command: "self turnkey"` fresh subagent. 도구가 없으면 사람/하네스 STOP |
...
| 스킬 | 분류 | 이유 |
|------|------|------|
| **next** | **제거 확정** | status와 **같은 `status_next.py` + 같은 입력**, status가 이미 `next_action: 반드시 1개` 출력 → next는 그 필드만 격리한 subset 뷰 = 순수 중복. 제거 scope = skill file + manifest/README/DESIGN 참조 + tests + installed cache parity. 단일이슈는 status, 트리는 orchestrate 루프(ready_leaves)가 대체 |
...
`verify-tool`/`review-tool`/`orchestrate.verify-command`/`orchestrate.review-mode`/
`orchestrate.review-command`. setup이 scaffold, doctor가 validate. 이 갱신이 없으면 스킬과 규칙이 발산.
...
- **F2 gear 신뢰 모델 개정**: B1 pre-stamp 폐기. gear는 start가 판단하고, orchestrate 모드에서는 work-agent가 보고한 값을 오케스트레이터가 read로만 사용한다. GitHub gear label write는 start 단일 writer.
