...
이 문서는 task-worker 분리 이후 `task-github` 플러그인의 **목표 아키텍처 정본**이다. 2026-07-14 현재 실제 소스 0.20.0은 DefinitionArtifact, local execution, orchestration과 GitHub integration을 한 플러그인에 함께 보유한다.
...
분리 완료 전에는 facade가 기존 동작을 유지해야 한다. task-worker 위임이 추가 agent/session hop을 만들거나 기존 ready set을 직렬화하면 migration 실패다.
