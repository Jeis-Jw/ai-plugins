...
선택 경계는 public plugin과 operation skill description에 노출한다. GitHub Issue Tree 기록이나
PR·CI·review·merge·closeout transport가 필요한 작업에 사용하며, local-only 실행이나 일반
상태 조회에는 provider adapter lifecycle을 붙이지 않는다.
...
- `task-github:init`: GitHub API를 호출하지 않고 `.task-github.yml`, local projection state 경로, gitignore를 멱등 초기화한다. 다른 기존 config는 `--force` 없이는 변경하지 않는다.
- `task-github:setup`: 자기 init 뒤 Git/GitHub repository와 label을 bootstrap한다. task-worker는 기존 config scaffold bridge로만 준비하며 새 public init을 직접 호출하지 않는다.
...
- execution-control bridge: task-worker의 permit decision/atomic claim/completion을 그대로 전달하고 immutable command receipt·verification evidence의 id+digest만 GitHub projection ref로 변환해 issue별 `execution_evidence` ledger에 멱등 기록
...
| verification evidence | task-worker immutable command receipt + verification evidence ref |
...
통합 흐름에서도 leaf delta receipt를 재사용할 수 있지만, merge로 새 integration head가 생기면 `integration-full` purpose의 최종 gate는 별도 physical identity로 남긴다. 따라서 ready sibling/worktree 병렬성과 child evidence 재사용은 보존하면서 root integration gate와 독립 review를 생략하지 않는다.
...
|---|---|---|
| `init` | provider config/local projection state 준비, remote mutation 금지 | 없음 |
