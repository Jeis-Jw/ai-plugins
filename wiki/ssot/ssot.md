...
- [[session-review-plugin]] — 설정된 snapshot provider(builtin|wiki-markdown|context-core) 위에서 worker/reviewer가 audit snapshot 또는 fast context와 reviewer lease로 리뷰를 수렴시키고 read-only doctor로 준비 상태를 확인하는 플러그인 설계 정본
