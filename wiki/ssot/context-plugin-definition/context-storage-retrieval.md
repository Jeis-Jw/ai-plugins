...
- `core_init | area_register | policy_install | index_fix`는 `source_type:"core_control"`, canonical `control_input`을 plan 안에 직접 가진다. control input은 각 명령의 normalized explicit args, target paths, descriptor/seed or marker/index diff digest만 포함하고 semantic attestation은 금지한다. 이 네 transition에는 owner-result fields와 validation receipt가 없어야 한다.
...
- `index_rebuild.derived_from`은 같은 plan의 effect ID를 중복 없이 가리킨다. 일반 artifact transition에서 `areas`는 그 effects가 touch한 area의 sorted set과 정확히 같고 `include_root:false`다. `transition: area_register|core_init`은 등록/init effect ID를 derived_from에 넣고 `include_root:true`를 요구한다. `index_fix`만 아래 별도 규칙을 따른다. 다른 hidden operation/effect는 `plan_preview_mismatch`다.
- `transition: index_fix`는 semantic effect 없이 `index_rebuild.derived_from:[]`를 허용한다. 이때 `areas`는 strict refresh가 drift를 확인한 sorted area set이고 root drift가 있을 때만 `include_root:true`다. preview는 rebuild 대상과 entry diff를 보여주며 artifact material/operation은 금지한다.
- `rename`은 byte-identical `file_move`, retire/supersede는 changed material을 가진 `file_move`, annotate/update는 `file_replace`, discard는 `file_delete`, index-only fix는 `index_rebuild`로 표현한다.
...
- process crash나 index write 실패 후 document가 남으면 문서를 정본으로 유지한다. 다음 `doctor`/`refresh --strict`가 partial edge/index drift를 잡고, caller가 보관한 bundle 재실행 또는 승인된 repair로 복구한다. exit 가능한 실패는 exit 6 `transaction_partial`/`index_stale`과 affected path·repair command를 반환한다.
- root index는 area install/remove 때만 쓰고 artifact capture hot path에서는 건드리지 않는다.
- 병렬 capture 후 최종 index는 문서 전체에서 결정적으로 재생성해 lost entry를 막는다.
