...
wiki-markdown 0.24.1은 이 워크스페이스의 AI-native 위키 메커니즘 정본이다.
...
- 0.24.0(4278097, 2026-08-13) — runtime hook을 두 host 모두에서 제거하고 init 조합 workflow를 `wiki` skill에 기술했다. raw vault init 뒤 `agent-policy`가 관리 블록에 best-effort capture 정책을 설치하는 설계였지만 독립 호출 surface는 누락됐다. 근거 [[DEC-2026-08-13-152825-capture-정책은-초기화-시-auto-loaded-agent-entry에-설치한다]].
- 0.24.1(2026-08-13) — `skills/init/`을 추가해 `$wiki-markdown:init`을 독립 노출하고 Codex default prompt도 실제 namespaced skill 이름으로 정렬했다. 기존 raw CLI와 `agent-policy`를 조합하므로 초기화의 멱등성·관리 블록 외 내용 보존 계약은 유지한다.
...
- `$wiki-markdown:init`: raw vault init + `agent-policy` skill을 통한 CLAUDE.md/AGENTS.md 관리 블록 스캐폴딩
...
- skills: `skills/wiki/SKILL.md`(런타임 cheat-sheet) + `skills/init/SKILL.md`(독립 초기화 command surface) + `skills/agent-policy/SKILL.md`(CLAUDE.md/AGENTS.md 스캐폴딩, `scripts/scaffold_agent_policy.py`)
