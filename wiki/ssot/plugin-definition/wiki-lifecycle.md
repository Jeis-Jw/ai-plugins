...
- **정본 위임**: 작업 플러그인 미연결이면 위키가 완료/미완 정본. 연결되면 **외부 작업 시스템(GitHub Issue/PR 등)이 정본**이고 위키 `done/`는 그 투영 — 동기화는 작업 플러그인 책임이며 위키 CLI는 외부 시스템을 호출하거나 상태를 해석하지 않는다. 연결된 외부 작업은 `relations.tasks` 링크로만 남긴다.
