...
이 워크스페이스의 AI-Native 위키 **메커니즘 정본**은 `plugins/wiki-markdown/` (marketplace plugin `jeis-ai-plugins/wiki-markdown@0.2.0`)이다. 본 영역(`wiki/ssot/plugin-definition/`)은 그 메커니즘의 **결정 그래프 anchor**와 **영역 라우팅** 역할을 하며, 5개 sub-ssot로 응집되어 있다. 메커니즘 세부는 plugin source(`SKILL.md`, `rules/knowledge-protocol.md`, `skills/wiki/references/wiki-protocol.md`) + 본 위키의 active decisions가 정본이다.
...
| [[wiki-four-layer-separation]] | mechanism/policy/agent entry/knowledge + agent-neutral + promotion threshold |

영역 외 ssot:

- [[agent-operating-model]] (`wiki/ssot/`) — 운영 정책 정본 (4계층 분리의 policy 계층)
...
- [[TRI-2026-05-29-105533-claude-md-as-policy-conflates-mechanism-and-policy]] — 4계층 분리의 근거
