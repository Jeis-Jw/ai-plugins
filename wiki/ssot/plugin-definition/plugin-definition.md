...
이 워크스페이스의 AI-Native 위키 **메커니즘 정본**은 `plugins/wiki-markdown/`이다. 버전·진화·소유범위의 단일 진입점은 [[wiki-markdown-plugin]]이며, 본 영역(`wiki/ssot/plugin-definition/`)은 그 메커니즘의 **세부 계약**을 다루는 **결정 그래프 anchor**와 **영역 라우팅** 역할을 하며 5개 sub-ssot로 응집되어 있다. 메커니즘 세부는 plugin source(`SKILL.md`, `rules/knowledge-protocol.md`, `skills/wiki/references/wiki-protocol.md`) + 본 위키의 active decisions가 정본이다.
...
| sub-ssot | 다루는 영역 |
|----------|-------------|
| [[wiki-data-model]] | 타입 체계 + ID 체계 + 관계 모델 + graph 밖 snapshot staging layer |
| [[wiki-lifecycle]] | Active/Retired + supersede + retire 2값 모델 + task/snapshot 경로 상태 |
| [[wiki-retrieval]] | 인덱스 파생 + 3-stage recall + search_terms + refresh 검사 + snapshot 조회 |
...
- **단일 CLI**: `skills/wiki/scripts/wiki_cli.py` (init / capture / retire / discard / complete / reopen / relate / snapshot / recall / refresh / schema)
- **계약 문서**: `skills/wiki/SKILL.md`, `skills/wiki/references/wiki-protocol.md`, `rules/knowledge-protocol.md`
- **템플릿**: `templates/{intent,decision,rejected_decision,trial_error,observation,ssot,runbook,task}.md`
...
- [[wiki-lifecycle]] — Record와 Living의 라이프사이클 정본: 경로 기반 active/retired, deprecated/superseded 2값 retire 모델과 완전삭제 discard(mistake-undo), supersede pair 양방향 저장, task 이진 상태(활성/done) + snapshot 루트 전용 휘발 staging. plugin-definition 영역의 sub-ssot.
- [[wiki-retrieval]] — 인덱스·조회·proactive context 정본: 폴더 단위 독립 인덱스, 3-stage recall, snapshot 조회, changed-path-stale, 승인형 capture와 초기화 시 auto-loaded agent policy 설치 계약.
