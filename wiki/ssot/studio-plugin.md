...
- Studio-owned `provider=session-review`가 unavailable이고 fallback이 native이면 기존 signed lease와 native dispatch를 섞지 않는다. cached capability로 `review-lease-replan-required`를 반환하고 `provider=native`인 새 canonical lease를 요구한다.
