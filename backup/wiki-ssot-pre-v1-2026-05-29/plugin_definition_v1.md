...
| retired_type | `deprecated` \| `superseded` 2값 (모든 record 공통, OBS 포함) | lifecycle 축을 "무효/대체"로 단일화. classified/classified_as 미도입 |
...
| stale 검사 | 타입별 시간 stale + `changed-path-stale`. observation은 시간 stale 미적용 | 시간 부패와 코드 변경 부패를 분리. 임시 관찰은 코드 drift만 추적 |
| 경로 영향 | `affects_paths` 선택 필드 + glob 매칭 | 코드 변경 시 관련 문서 재검토를 능동 유도 |
| 검색 보조 | `summary`, `tags`, 본문 ripgrep, 선택 `search_terms` (recall Stage 1 매칭) | 강제 없이 검색 누락 대응용 escape hatch 제공 |
| 인덱스 파생 | 폴더 단위 독립 — 재귀 = 폴더 발견, 비재귀 = 노트 수집 | 상위 인덱스 비대화 방지. nested ssot 도입 취지(분할)와 일관 |
| ssot 구조 | 하위 폴더 허용 + basename 전역 유일성 | 비대화 시 분할 가능성을 열되 resolver 단순성 유지 |
| refresh | 리포트 기본, `--fix`는 `index`/`retired-in-index` 화이트리스트 인자만, `duplicate-basename`/`empty-lesson`/`changed-path-stale` 검사 | 안전한 자동수정만 허용하고 의미 판단은 운영자에게 유지 |
| recall | 3-stage 조회 + `--read a,b,c` batch read | 명시 context 묶음을 순서 보존해 읽기 |
| 승격 기준 | 추상 promotion threshold는 plugin spec (§13.6), 운영 트리거는 operating model | plugin은 구조 검증만, 기록 가치 판단은 정책 계층에서 수행 |
| 운영 계층 | mechanism / policy / agent entry / knowledge 4계층 분리 | 플러그인 메커니즘과 에이전트 운영 규칙의 결합을 낮춤 |
| agent-neutral | CLI / 스키마 / 알고리즘 출력에 agent 이름 없음. 본문 산문의 원칙 설명·policy 계층 예시는 허용 | 미래 도구 호환성. 운영 규약은 policy 계층으로 격리 |
...
7. **ADR-compatible, not ADR-limited.**
   decision/rejected_decision은 ADR(Architecture Decision Record)의 결정·대안 추적과 대응하지만, intent/trial_error/observation까지 포함해 운영 기억을 더 넓게 다룬다.
...
| stale 의미 | 시간·경로 변경 모두 재검증 대상 | supersede가 주된 유효성 판정. 시간/경로 stale은 타입별 적용 (§7 표) |

*취지*: 이 구분이 context를 별도로 묶은 이유(=의사결정 *맥락 엔진*)와 정확히 맞물린다. "지금 어떻게?"는 ssot 본문, "왜/어떻게 변해왔나?"는 context 추적.

stale-check의 타입별 적용 범위는 §7에 표로 명시한다.
...
search_terms: [...]          # 선택. recognized optional field. recall Stage 1 매칭
---
```
...
| 필드 | 범위 | 의미 |
|------|------|------|
| `verified_at` | **living 권장, trial_error/observation 선택, intent/decision/rejected_decision 없음** | "현재 유효함을 마지막 확인한 날" |
| `affects_paths` | **ssot/runbook/trial_error/observation 선택** | 관련 코드 경로 (glob 허용: `src/auth/**`) |
| `search_terms` | **모든 타입 선택** (recognized optional field) | 검색 보조 키워드. capture 기본 생성 X, refresh 검사 X, recall Stage 1 매칭 O |
...
| `tags` | 자유 태그는 드리프트로 검색 정확성↓ → **통제 어휘**: `ssot/tag-vocabulary.md`의 `## 어휘` 목록이 허용 집합. refresh가 어휘 밖 태그를 플래그 (어휘 문서 없으면 검사 skip) |
| `verified_at` | "사실/설계는 부패한다." living(ssot/runbook)에 특히 중요. record 중에서는 trial_error/observation만 의미. intent/decision/rejected는 supersede로만 유효성 판정 |
| `affects_paths` | 코드 변경과 문서의 능동적 drift 감지(§13.5). refresh의 `changed-path-stale` 검사 기반 |
| `search_terms` | summary+tags+본문 ripgrep 외의 escape hatch. capture/refresh 무관, recall Stage 1 매칭 |
...
*취지*: intent/decision/rejected는 supersede 여부로만 유효성 판정. timestamp 기반 stale은 living + 함정/관찰 영역. observation은 임시 상태라 시간 stale 무의미(분류 강제는 §18 OBS aging 후속에서 검토). changed-path-stale은 코드 drift trigger라 affects_paths 보유 타입(living + 함정/관찰) 모두 적용.
...
recall Stage 1 매칭 대상에 포함되므로, search_terms에만 키워드가 들어있어도 검색에서 발견된다.
...
```
재귀 = 폴더 발견 방식 (vault를 재귀 탐색해 인덱스가 필요한 폴더를 모두 찾음)
비재귀 = 각 인덱스의 notes 수집 (각 인덱스는 자기 폴더의 직속 문서만 모음)
```

- 각 `<folder>.md` 인덱스는 **해당 폴더의 직속 문서만** 포함한다.
- **하위 폴더 문서는 하위 폴더의 독립 인덱스에만** 포함된다.
- 상위 인덱스는 필요하면 하위 인덱스 링크를 별도 섹션으로 노출할 수 있지만, **하위 문서 summary를 중복 수집하지 않는다.**
- refresh / 인덱스 파생은 vault를 재귀 탐색해 각 폴더 인덱스를 생성하되, 각 인덱스의 notes 수집은 비재귀로 한다.

*취지*: nested ssot 도입의 의도가 *문서 비대화 방지*인데, 상위 인덱스가 하위 문서를 모두 끌어오면 다시 비대해진다. 같은 문서가 부모/자식 인덱스에 중복 노출되는 것도 방지. 계층적 조회 모델(Index → Summary → Section → Full Text)에는 폴더 단위 독립이 더 잘 맞는다. 전체 조망은 루트 README의 "이런 질문 → 이 폴더/인덱스" 라우팅이 담당한다.

### 인덱스 형식

- 형식: `- [[<basename>]] — {summary}`
- 정렬: 파일명 오름차순
- `retired/` 문서·인덱스 파일은 제외
...
- **예외: `relations.tasks`는 외부 작업 시스템 참조.** MVP 값은 GitHub issue ref(`owner/repo#N`). 위키 파일 존재 검사 대상 아님. 작업 시스템 연동 plugin이 별도 resolver를 제공하면 이 형식은 확장 가능.
...
*observation 취지*: 실행 중 발견은 정본(ssot/runbook) 또는 결정(decision)·task에 닿는다. intent에 직접 닿는 경우는 드묾(추상 원칙과 거리 멂) — 필요하면 후속 decision/trial_error가 intent와 연결한다.
...
- **agent-neutral**: CLI 인자/출력 메시지에 Claude/Codex 등 특정 도구 이름을 사용하지 않는다(§17 반려).
...
- **observation은 ssot/runbook/decisions/tasks 관계 가능**. intent 관계는 허용 안 함 → exit 2.
...
- **promotion threshold 검증**: type별 구조 조건(§8 고정 섹션 존재, 필수 frontmatter)이 충족 안 되면 거부. 의미적 승격 가치 판정은 안 함(§13.6).
...
- `--type superseded`이면 `--superseded-by` 필수. 해당 참조는 basename으로 해소·존재 검증하며, **active context/* record**여야 한다(ssot/runbook이면 exit 2).
...
```
refresh [--check <name|all>] [--days <N>] [--changed-path <path,..>]
        [--path <subdir>] [--fix <index|retired-in-index,..>]
...
- **검사**:
  - `stale`: living + (verified_at 있는 trial_error)의 verified_at > N일. **observation은 시간 stale 미적용** (changed-path-stale만, §7 표).
  ...
  - **`changed-path-stale`**: `--changed-path` 또는 git diff 경로가 `affects_paths`(glob) 매칭이고 `verified_at` 미갱신 (대상: living + trial_error/observation). 단순 코드 read는 ssot 갱신 불요 — 의도된 트레이드오프.
  - **`duplicate-basename`**: vault 전역 basename 중복 검사. 인덱스 파일과 `retired/` 문서도 resolver 모호성을 만들 수 있으므로 검사 대상에 포함.
  - **`empty-lesson`**: trial_error의 `## 교훈` 섹션이 placeholder만이거나 비어있음 플래그. observation으로 권장하는 운영 신호.
- **`--changed-path` 동작**:
  - 없으면 git diff에서 변경 경로를 자동으로 구한다.
  - 있으면 CI / test fixture용 명시 입력으로 사용한다.
- **`--fix` 화이트리스트**:
  - 허용 인자: `index`(인덱스 동기화), `retired-in-index`(인덱스에서 retired 제거).
  - 반드시 인자 명시 — **bare `--fix`는 exit 2 거부**.
  - 그 외 인자(예: `--fix broken-rel`) 시도 → exit 2 거부.
  - 실행 시 stdout/JSON에 변경 내역 보고(silent 변경 금지).
...
```
정식 record로 승격되는 정보는 장기 재사용 가능성, 구조적 영향, 반복 가능성,
되돌리기 비용, 후속 작업자가 알아야 할 필요성 중 하나 이상을 가져야 한다.

plugin은 승격 판단 자체를 자동화하지 않는다.
plugin은 capture된 문서가 타입별 구조 조건과 스키마를 만족하는지만 검증한다.
```

운영 트리거(예: leaf issue 작성 시 어떤 후보를 capture할지, 누가 DEC/TRI로 승격할지)는 **`wiki/ssot/agent-operating-model.md`의 영역** — agent-neutral 유지를 위해 plugin spec에는 두지 않는다.

### 13.7 promote — sandbox → 정식 (보류, 계약만)

- sandbox 노트의 결론을 추출해 정식 문서 생성 + sandbox에 산출물 basename 기록.
- **생성 가능 타입**: `INT / DEC / REJ / TRI / OBS / ssot / runbook` 전 타입.
- promotion threshold(§13.6)의 **구조 조건만 검증**, 운영 판정은 안 함(agent-operating-model 영역).
...
for folder in vault 내 모든 인덱스 보유 폴더 (재귀 탐색, retired/ 제외):
  ...
  # nested 폴더는 자신의 독립 인덱스를 별도로 가짐 (notes 수집은 비재귀)
  for d in notes: summary = frontmatter(d).summary
  index 본문 "## 노트" 섹션을 "- [[{basename(d)}]] — {summary}" 목록으로 갱신(멱등)
```

상위 인덱스는 필요하면 하위 인덱스 링크만 별도 섹션으로 노출할 수 있으나, 하위 문서 summary를 중복 수집하지 않는다.
...
new는 active context/* record여야 한다 (ssot/runbook은 successor 불가)
...
changed-path-stale:     --changed-path 또는 git diff 경로 vs affects_paths(glob), verified_at 미갱신
duplicate-basename:     vault 전역 basename 중복 (인덱스·retired/ 포함)
...
### observation (후속 TRI로 승격되어 retire된 상태, `retired/` 내)
```yaml
---
title: webhook 처리 타임아웃 리스크
created_at: 2026-05-28
...
| **상위 인덱스가 하위 폴더 문서까지 재귀 수집** | nested ssot 도입 의도(분할)와 충돌. 부모/자식 인덱스 중복 노출, 부모 인덱스 비대화 → 폴더 단위 독립 채택(§10) |
...
### 19.2 인덱스 파생 — 폴더 단위 독립 (§14.2)
- summary 가진 문서 N개 폴더 → `<폴더명>.md` "## 노트"에 `- [[basename]] — summary` N줄, 파일명 오름차순.
- **nested 독립**: `ssot/auth/auth.md`는 `ssot/auth/` 직속 문서만 포함하고, `ssot/`의 다른 폴더 문서는 포함하지 않음.
- **상위 인덱스 중복 수집 금지**: `ssot/ssot.md`는 `ssot/auth/auth-session.md` 같은 하위 폴더 문서를 summary로 직접 수집하지 않음.
- `retired/` 문서는 인덱스에 없음.
- 재실행 결과 동일(멱등).
...
- `--superseded-by`가 ssot/runbook을 가리키면 exit 2. successor는 active context/* record여야 한다.
...
### 19.8 refresh 추가 검사 — changed-path-stale / duplicate-basename / empty-lesson
- ssot의 `affects_paths: [src/auth/**]` + 해당 경로 git 변경 + `verified_at` 미갱신 → changed-path-stale 1건.
- `--changed-path src/auth/session.ts` 명시 입력 + 동일 매칭 → changed-path-stale 1건 (CI/test fixture 경로).
- `--changed-path` 미지정 + git diff 자동 추출 → 동일 결과.
- vault 전역에 같은 basename 2개 → duplicate-basename 1건.
- trial_error의 `## 교훈`이 placeholder/공백 → empty-lesson 1건.

### 19.9 refresh --fix
- `--fix index` 또는 `--fix index,retired-in-index` 허용.
- **bare `--fix`** (인자 없음) → exit 2 거부.
- `--fix broken-rel`, `--fix stale` 등 화이트리스트 외 인자 → exit 2 거부.
- 실행 시 변경 내역 stdout/JSON 보고(silent 변경 금지).

### 19.10 promote 타입 범위 (§13.7)
- promote가 생성할 수 있는 타입: `INT / DEC / REJ / TRI / OBS / ssot / runbook` 전 타입 명시.
- 구조 조건 검증만, 의미적 승격 가치 판정은 안 함.

### 19.11 agent-neutral 원칙
- 스킬 **CLI 인자/출력 메시지**에 "Claude"/"Codex" 등 특정 agent 이름 없음.
- **frontmatter 필드명, 알고리즘 명세, 스키마 정의**에 agent 이름 없음.
- 본문 산문에서 agent-neutral 원칙을 설명하거나 운영 정책 영역(§15)을 설명하기 위한 agent 이름 언급은 허용 (예: §15 "Claude/Codex/GitHub 역할은 agent-operating-model.md").
...
인덱스      <폴더명>.md (summary 투영, 파생), 폴더 단위 독립 — 재귀=폴더 발견, 비재귀=노트 수집
...
스킬        init / capture / retire / recall(3-stage + batch read + search_terms 매칭) / refresh(점검 + --fix 화이트리스트 인자) / promotion threshold / promote · sandbox(보류)
refresh CLI refresh [--check] [--days] [--changed-path <path,..>] [--path] [--fix <index|retired-in-index,..>] [--strict] [--vault] [--json]
promote     INT / DEC / REJ / TRI / OBS / ssot / runbook 전 타입 생성 가능 (구조 검증만)
4계층      mechanism(plugin) / policy(agent-operating-model.md) / agent entry(CLAUDE.md, AGENTS.md) / knowledge(wiki/*)
원칙        plugin은 agent-neutral. agent 이름은 policy 영역으로 격리 (CLI/스키마/알고리즘 출력 0건, 본문 산문의 원칙 설명·policy 계층 예시는 허용). promotion 판정은 운영자
```
