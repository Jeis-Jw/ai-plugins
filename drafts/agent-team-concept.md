...
> 상태: session-review co-design 라운드 (converge). r1(explore)은 approved —
> 이 판은 r1 피드백 4건의 [should-reflect]와 directional을 반영하고,
> **롤 구성**과 **제어 모델**을 개발 착수 가능한 계약 수준까지 내린다.
...
tools: [WebSearch, Read]    # 허용 도구 화이트리스트
activation: always          # always | gated:<조건>  (예: 마케팅=ship 게이트 후)
---
...
meeting_output:
  ...
  delta_log: [{round, changed_what, evidence, rejected_alternative}]
...
- **독립 판정자 (delta-scorer)**: 참석자가 아닌 별도 에이전트. prior 없음,
  임무는 채점뿐. 매 라운드 delta 레코드를 검증하고 dry/alive를 판정한다.
  참석자 하니스의 자기 판정을 신뢰하지 않는다.
...
| delta-scorer | 판정 | 없음 (기능적) | 페르소나 아님 — 미팅 파라미터로 주입 |
...
converge: 합성 에이전트가 synthesis + minority + delta_log 생성
```
...
### 이번 판에 반영된 결정 (r1 피드백 수용)

- 소집형 (상주는 제한적 최적화)
- 브로커: 리추얼별 스크립트 2개 + 공통 I/O 계약 선통일
- 독립 판정자(delta-scorer) + delta 기반 연극 판정
- discussions/는 합성본+delta만, raw는 TTL
- MVP 검증에 baseline 비교, 미션은 산출물 단순한 것
- CEO 금지에 판단 대리 합성 추가

### 남은 논점 (r2 co-design 안건)

1. **네이밍 확정**: `studio` 우선 후보 (r1 reviewer 추천). 사용자 게이트 사안.
2. **판정자 모델 티어**: delta-scorer를 상위 모델로 둘 가치가 있나, 동일 티어로
   충분한가.
3. **pairing과 session-review의 관계**: 페어링 브로커를 신규로 갈지(가벼움,
   인메모리), session-review 루프를 재사용할지(무겁지만 audit 가능). 초안 권고:
   MVP는 신규 경량 브로커, session-review는 사람 게이트가 필요한 리뷰에만.
4. **team/의 위치**: 미션 대상 consumer repo 안 `team/`인가, 별도 전용
   워크스페이스인가. 초안 권고: consumer repo 안 (산출물과 같은 곳).
5. **MVP 실험 미션 선정**: 문서 미션 vs 소형 CLI 미션 — 구체 후보 필요.
6. **페르소나 tools 강제 수단**: 선언만 두고 프롬프트 규범으로 갈지, 하니스
   권한으로 실제 차단할지. 초안 권고: MVP는 프롬프트 규범 (하니스 차단은
   플러그인화 때).
